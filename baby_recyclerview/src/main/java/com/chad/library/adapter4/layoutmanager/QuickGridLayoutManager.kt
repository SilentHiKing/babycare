        ...
        override fun getSpanSize(position: Int): Int {
            ...
            if (adapter is ConcatAdapter) {
                ...
                return when (adapter) {
                    is FullSpanAdapterType -> {
                        spanCount
                        Log.d("hello","spanCount = ${spanCount}")
                    }
                    is BaseQuickAdapter<*, *> -> {
                        ...
                        if (adapter.isFullSpanItem(type)) {
                            spanCount.also {
                                Log.d("hello","1 spanCount = ${spanCount}")
                            }
                        } else {
                            (originalSpanSizeLookup?.getSpanSize(position) ?: 1).also {
                                Log.d("hello","2 spanCount = ${it}")
                            }
                        }
                    }
                    else -> (originalSpanSizeLookup?.getSpanSize(position) ?: 1).also { Log.d("hello","3 spanCount = ${it}") }
                }
            }
        }
    }
