...
BabyCare 是婴儿生活记录与规律洞察工具，不是装饰型相册应用。疲劳、夜间、单手操作和快速判断是高频使用约束，不能替代“记录生活规律、理解规律、辅助预测、促进健康成长”这个核心。
...
普通选择控件统一采用 **Quiet Selection**：`control_surface_default` 白底 + 0.1dp 品牌描边 + 品牌文字，不做白字实色块、底部粗色条或多层浅色填充。业务功能色只用于图标、图表和数据语义，不能同时参与选中边框或选中文字。保存按钮和真正主行动按钮例外，继续使用品牌实色底。

喂养页的辅食分类和子类型位于喂养类型下方，选中态必须参考上方喂养类型分段控件：`control_surface_default` 白底、`?attr/colorBrand` 描边、`?attr/colorBrand` 文字。辅食 Chip 文字复用 `selector_radio_text_color`，不得另建一套 `feeding_primary`、业务色描边或固定蓝色选中态，避免同页出现两套状态色。
...
记录事件详情里的字段标签（如“大便性状”“测量部位”“疫苗名称”“备注”）统一使用 `EventDetailFieldLabel`：12sp、`colorTextSecondary`，标签到输入/选项使用 `event_detail_label_option_gap`，相邻字段组使用 `event_detail_field_gap`。字段标签只承担分组说明，输入内容和选项仍使用正文层级，避免标签与可操作选项层级过近。
