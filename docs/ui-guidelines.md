...
- `control_border_default #E3ECEB`：未选中控件和输入框边框。
- `event_icon_surface #FFF2D9`：通用事件图标承托面。

选中态默认是浅暖底 + 1dp 品牌/语义描边 + 品牌/语义文字，不做白字实色块。保存按钮和真正主行动按钮例外，继续使用品牌实色底。
...
洞察区、汇总区和说明区是第二层，应使用独立区块标题和统一 `surface + stroke` 卡片样式。区块之间用 24dp 左右的垂直距离建立清晰分段，区块内部卡片之间保持 12-16dp 间距，避免页面看起来是一整片无边界内容。

指标块用于承载短数字、状态和类别占比，统一使用浅底、1dp 边框、8dp 圆角。图标可以放在浅色承托面内，数字使用业务语义色，说明文字使用弱化文本色，不能用大面积彩色底或阴影制造层级。
